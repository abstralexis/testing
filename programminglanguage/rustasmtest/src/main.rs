...
fn run_line(line: &Vec<String>, mem: &mut HashMap<String, i32>) -> () {
    ...
    */
    ...
    if !KEYWORDS.contains(&first) { panic!("Invalid keyword") }

    if line[0] == "iadd" {
        let a = line[1].parse::<i32>().unwrap_or_else(|_| get_val(&line[1], mem));
        let b = line[2].parse::<i32>().unwrap_or_else(|_| get_val(&line[2], mem));
        let val = iadd(&a, &b);
